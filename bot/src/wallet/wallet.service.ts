...
export class WalletService {
  ...
  getToken2022Balance = async (
    ...
    try {
      ...
      throw new Error(`Failed to fetch Token-2022 balance: ${error}`);
    }
  };
  ...
  transferSPLToken = async (
    ...
    try {
      ...
      if (!recipientAccountInfo) {
        console.log('Recipient ATA does not exist, appending creation instruction...');
        transaction.add(
          createAssociatedTokenAccountInstruction(
            senderKeypair.publicKey,
            recipientATA,
  ...
  getEVMNativeTokenBalance = async (
    address: string,
    rpc: string,
  ): Promise<Record<string, number>> => {
